...
get_host_port() {
    HOST_PORT=$(docker container inspect jusan-docker-mount | jq --raw-output '.[0]."HostConfig"."PortBindings"."81/tcp"[0]."HostPort"')
    echo $HOST_PORT
}

is_port81_present() {
    docker container inspect jusan-docker-mount | grep "\"81/tcp\": \[" > /dev/null
    echo $?
}
...
if [ $(is_port81_present) -eq 0 ] ; then echo ✅ ; else echo "❌ неправильный порт контейнера" && exit 1 ; fi
